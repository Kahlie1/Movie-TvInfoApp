...
import { fetchTv } from "../Services/Services";
...
const TvShows = ({ navigation }) => {
    ...
    useEffect(() => {
        setLoading(true);
        fetchTv(searchTerm, tvShows).then((data) => {
            setTvShows(data);
            setLoading(false);
        });
    }, [searchNow]);

    const getGenres = tvShows.map(({ name, genre_ids }) => {
                                ...
                                return (
                                    <View style={styles.flView}>
                                        <Card style={styles.movieCard}>
                                            <TouchableOpacity
                                                onPress={() => {
                                                    navigation.navigate('Tv', { tv: item, genre: getGenres[index].genres });
                                                }}>
                                                <Image
                                                    source={{
